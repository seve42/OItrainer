...
(function(global){
  ...
  class StudentContestState {
    constructor(student, problems){
      ...
      this.skippedProblems = new Set(); // 已跳过的题目ID集合，不会再回头做
    }
    ...
    // 获取未完成的题目列表（排除已跳过的题目）
    getUnsolvedProblems(){
      return this.problems.filter(p => !p.solved && !this.skippedProblems.has(p.id));
    }
  }
    ...
    simulateStudentTick(state){
      ...
      if(success){
        ...
        if(shouldSkip){
          ...
          // 将当前题目标记为已跳过，不会再回头做
          state.skippedProblems.add(state.currentTarget);
