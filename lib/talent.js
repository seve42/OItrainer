          ...
          handler: function(student, eventName, ctx){
            try{
              if(eventName === 'mock_start'){
                student._talent_backup = student._talent_backup || {};
                // 使用独立的原始值备份key，避免与其他天赋冲突
                if(!student._talent_backup['原题机（伪）_original']){
                  // 备份当前值（可能已被其他天赋修改）作为基准
                  student._talent_backup['原题机（伪）_original'] = { 
                    thinking: Number(student.thinking || 0), 
                    coding: Number(student.coding || 0), 
                    mental: Number(student.mental || 0) 
                  };
                  // 在当前值基础上翻倍（叠加效果）
              ...
              if(eventName === 'mock_end'){
                // 恢复到原题机伪触发前的值
                if(student._talent_backup && student._talent_backup['原题机（伪）_original']){
                  const bak = student._talent_backup['原题机（伪）_original'];
                  student.thinking = bak.thinking; 
                  student.coding = bak.coding; 
                  student.mental = bak.mental;
                  delete student._talent_backup['原题机（伪）_original'];
                  if(student._talent_state) student._talent_state['原题机（伪）'] = false;
                  return { action: 'mock_cleanup', message: '原题机（伪）：模拟赛效果清零并还原能力' };
                }
              }
        ...
        handler: function(student, eventName, ctx){
          ...
          try{
            ...
            if(eventName === 'mock_end' || eventName === 'mock_contest_finish'){
              ...
              // 再恢复激进（如果存在）
              if(student._talent_backup && student._talent_backup['激进_original']){
                const bak = student._talent_backup['激进_original'];
                student.thinking = bak.thinking;
