...
class GameState {
  ...
  updateWeather(){
    ...
    try{
      ...
      // 游戏周数对应：第1周=9月（秋季），第9周=12月（冬季），第17周=次年3月（春季），第23周=5月（春末/夏初），第25周=6月（夏季），第32周=8月（夏季）
      // 32周游戏对应一个学年：9月-次年8月
      const weeksPerYear = SEASON_WEEKS || 32;
      const weekInYear = ((this.week - 1) % weeksPerYear) + 1;
      
      // 第1-8周：秋季（9-11月）
      if(weekInYear >= 1 && weekInYear <= 8){ 
        if(this.is_north) this.temperature = uniform(5,20); 
        else this.temperature = uniform(15,28); 
      }
      // 第9-16周：冬季（12-2月）
      else if(weekInYear >= 9 && weekInYear <= 16){ 
        if(this.is_north) this.temperature = uniform(-10,5); 
        else this.temperature = uniform(5,15); 
      }
      // 第17-24周：春季（3-5月）
      else if(weekInYear >= 17 && weekInYear <= 24){ 
