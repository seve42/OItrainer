...
function parseResultCsvContent(raw){
  try{
    ...
    const threshold = Math.max(maxMatchId - 9,0);
    ...
    for(const row of rows){
      ...
      if(_namePoolsByProvince[provId].length<150){
        _namePoolsByProvince[provId].push({ name, weight: Math.pow(Math.max(0, Number(score) || 0),0.9) });
