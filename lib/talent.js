...
(function(global){
  const TalentManager = {
    ...
    tryAcquireTalent(student, multiplier = 1.0) {
      ...
      try{
        ...
        if(Math.random() >= (isFinite(globalGetTalentProb) ? globalGetTalentProb : 0.5)){
      ...
      if (student.talents.size < MAX_TALENTS) {
        for (const tDef of registered) {
          ...
          if (prob <= 0) continue;
          if (Math.random() < prob * multiplier) {
      ...
      if (negativeTalents.length > 0) {
        ...
        if (Math.random() < (isFinite(globalGetTalentProb) ? globalGetTalentProb : 0.5)) {
          // 随机选择一个负面天赋移除
          const removedTalentName = negativeTalents[Math.floor(Math.random() * negativeTalents.length)];
    ...
    assignTalentsToStudent(student, multiplier = 1.0) {
      ...
      try{
        const globalGetTalentProb = (typeof GET_TALENT_Probability !== 'undefined') ? Number(GET_TALENT_Probability) : (typeof window !== 'undefined' && typeof window.GET_TALENT_Probability !== 'undefined' ? Number(window.GET_TALENT_Probability) : 0.5);
        if(Math.random() >= (isFinite(globalGetTalentProb) ? globalGetTalentProb : 0.5)){
          return; // 未通过总体概率门槛
        }
  ...
  checkAndHandleTalentLoss(student) {
    ...
    if (Number(student.pressure || 0) >= TALENT_LOSS_THRESHOLD) {
      if (Math.random() < TALENT_LOSS_PROB) {
        // 第一步：只从正面天赋中选择进行丧失
        ...
        if (positiveTalents.length > 0) {
          const lostTalentIndex = Math.floor(Math.random() * positiveTalents.length);
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
          ...
          handler: function(student, eventName, ctx){
            try{
              ...
              if(Math.random() < 0.10){
                ...
                if(others.length > 0){
                  const randomK = others[Math.floor(Math.random() * others.length)];
            ...
            try{
              ...
              if(eventName === 'outing_finished'){
                ...
                if(ctx && ctx.knowledge_gain){
                  ...
                  const allKnowledge = ['knowledge_ds','knowledge_graph','knowledge_string','knowledge_math','knowledge_dp'];
                  const randomK = allKnowledge[Math.floor(Math.random() * allKnowledge.length)];
        ...
        this.registerTalent({
          name: '省钱大师',
          description: '规划开支能手：若学生参加外出集训，则本次集训开支直接减少 5000（需外部系统在结算时应用）。',
          color: '#43A047',
          prob: 0.06,
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(eventName === 'outing_finished' || eventName === 'outing_cost_calculate'){
                ...
                // 这里按需求改为固定生效（如果需要概率，可改为 Math.random() < 0.2）
        ...
        this.registerTalent({
          ...
          handler: function(student, eventName, ctx){
            try{
              // 监听模拟赛结束事件（付费模拟也会触发 mock_contest_finish）
              if(eventName === 'mock_contest_finish' || eventName === 'mock_end'){
                // 直接增加额外的知识点
                const allKnowledge = ['knowledge_ds','knowledge_graph','knowledge_string','knowledge_math','knowledge_dp'];
                for(const k of allKnowledge){
                  const current = Number(student[k] || 0);
                  student[k] = current + current * 0.5; // 额外增加50%
                }
        ...
        this.registerTalent({
          name: '庸才',
          description: '（Meta 负面）资质平平，在训练中无法领悟任何新的天赋。',
          color: '#9E9E9E',
          prob: 0.03,
          kind: 'negative',
          beneficial: false,
