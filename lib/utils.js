...
function getLetterGrade(val) {
  ...
  if (n > 100) {
    ...
    // 新规则：让 val 在 110-210 区间按 U1e..U1sss 均匀映射，210-310 映射到 U2e..U2sss
    // 110 -> U1e, 210 -> U1sss（210 映射到最后一项）；210 -> U2e, 310 -> U2sss
    const v = Number(val);
    if (v >= 110 && v < 210) {
      const rel = (v - 110) / 100.0; // [0,1)
      let idx = Math.floor(rel * subs.length);
      if (idx < 0) idx = 0; if (idx >= subs.length) idx = subs.length - 1;
      return `U1${subs[idx]}`;
    }
    if (v >= 210 && v < 310) {
      const rel = (v - 210) / 100.0;
      let idx = Math.floor(rel * subs.length);
      if (idx < 0) idx = 0; if (idx >= subs.length) idx = subs.length - 1;
      return `U2${subs[idx]}`;
    }
    // 其余大于 100 的值保持原有按整数步进的映射（向后兼容）
