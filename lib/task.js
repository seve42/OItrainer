...
function handleSelfStudy(student, task, gameState) {
  ...
  const thinking_gain = getRandom() * 0.1 * Math.max(0, (100 - student.thinking)) / 100 * abilityDecayFactor;
  const coding_gain = getRandom() * 0.1 * Math.max(0, (100 - student.coding)) / 100 * abilityDecayFactor;
  if(typeof student.addThinking === 'function') student.addThinking(thinking_gain);
  else student.thinking = (student.thinking || 0) + thinking_gain;
  if(typeof student.addCoding === 'function') student.addCoding(coding_gain);
  else student.coding = (student.coding || 0) + coding_gain;
