...
(function(global){
  ...
  function generateInternationalStudents(chineseStudents){
    ...
    const countries = [
      '中国','韩国','罗马尼亚','美国','加拿大','保加利亚','新加坡','越南','匈牙利','伊朗','日本','波兰','马来西亚','台湾','哈萨克斯坦','埃及','克罗地亚','澳大利亚','印度尼西亚','塞尔维亚','瑞士','土耳其','乌兹别克斯坦','荷兰','北马其顿','新西兰','吉尔吉斯斯坦','斯洛伐克','亚美尼亚','巴西','法国','印度','泰国','意大利','巴基斯坦','瑞典','比利时','菲律宾','墨西哥','澳门','德国','奥地利','古巴','格鲁吉亚','哥斯达黎加','爱尔兰','立陶宛','捷克共和国','爱沙尼亚','沙特阿拉伯','摩洛哥','波黑','斯洛文尼亚','挪威','智利','秘鲁','突尼斯','厄瓜多尔','萨尔瓦多','多米尼加共和国','卢旺达','加纳','尼日利亚'
    ];
  ...
  const scoreDisplay = scoreNum > 0 ? scoreNum.toFixed(0) : '0';
        ...
        // IOI 也显示按奖牌分组的结果（若 r.medal 存在则显示），否则回退到晋级/未晋级信息
        else if(config && config.name === 'IOI'){
          if(r.medal === 'gold') resultText = '<span style="color:#e6b422;font-weight:700">🥇 金牌</span>';
          else if(r.medal === 'silver') resultText = '<span style="color:#b0b0b0;font-weight:700">🥈 银牌</span>';
          else if(r.medal === 'bronze') resultText = '<span style="color:#cd7f32;font-weight:700">🥉 铜牌</span>';
          else resultText = r.passed ? '<span style="color:#38a169;font-weight:700">晋级</span>' : '<span style="color:#999">未晋级</span>';
        }
        // 其他比赛保持原来的晋级显示
        else {
          ...
          if(hasGoldMedal && config && config.name === 'NOI'){
            try{
              ...
              if(typeof window.showNationalTeamChoice === 'function'){
                window.showNationalTeamChoice(noiResults, noiMaxScore || 700);
              }
            }catch(e){
