...
(function(global){
   ...
   */
  function generateSubtasks(totalScore, problemDifficulty, thinkingBase=null, codingBase=null){
    const numSubtasks = 3 + Math.floor(Math.random() * 3); // 3-5档
    const subtasks = [];
  ...
  // 不再对思维/代码难度进行上限 100 限制，允许超过 100，以避免在高难度比赛中被提前截断。
  ...
  const thinkingDifficulty = Math.floor(thinkingDifficultyBase * (1.0 + (typeof THINKING_DIFFICULTY_BONUS === 'number' ? THINKING_DIFFICULTY_BONUS : 0.0)));
  const codingDifficulty = Math.floor(codingDifficultyBase * (1.0 + (typeof CODING_DIFFICULTY_BONUS === 'number' ? CODING_DIFFICULTY_BONUS : 0.0)));
  ...
  class ContestSimulator {
    ...
    selectProblem(state, student){
      ...
      const scored = unsolved.map(p => {
        ...
        // 基础评分：知识加成 - 难度差距
        let baseScore = knowledge * 3 + ability - Math.abs(currentSubtask.difficulty - ability) * 0.5;
    ...
    attemptSubtask(student, problem, subtask){
      ...
      // 思维能力判定（thinking）：以 student.thinking 与 knowledge 为主
      const thinkingBase = Number(student.thinking || 50) + knowledge * 2.0;
  ...
  const thinkingGap = thinkingBase - taskThinkingDifficulty;
      ...
      // 代码能力判定（coding）：以 student.coding 为主，知识提供少量加成
      const codingBase = Number(student.coding || 50) + knowledge * 0.8;
      ...
      let codingProb = 1.0 / (1.0 + Math.exp(-codingGap / 12.0));      // coding 稳定性受心理影响较小，给出较窄区间
      ...
      try{
        const effectiveAbility = ability + knowledge * 2.0;
      ...
      try{
        ...
        this.addLog(`${student.name} 尝试 T${(problem.id||0)+1} 档位：思维通过 ${thinkingPass} (${tPct}%)，代码通过 ${codingPass} (${cPct}%)`, 'info', student.name);
      }catch(e){}
  ...
  // 题目难度（基于比赛难度和题号）
  const baseDifficulty = contestDef.difficulty;
  const rawProblemDifficulty = baseDifficulty + (i * 20) - 10 + Math.floor(Math.random() * 20);
  // 归一化到 0-100 范围，使用常量 DIFFICULTY_NORMALIZE_DIVISOR（定义在 constants.js）
  let problemDifficulty = rawProblemDifficulty / (typeof DIFFICULTY_NORMALIZE_DIVISOR !== 'undefined' ? DIFFICULTY_NORMALIZE_DIVISOR : 4.0);
  problemDifficulty = Math.max(0, Math.min(100, Math.floor(problemDifficulty)));
  ...
  // 不再对值进行 0-100 的截断，允许超过 100 以表现高难度的放大效应。
