...
(function(global){
  ...
  function holdMockContestModalNew(isPurchased, difficultyConfig, questionTagsArray){
    ...
    // 在模拟赛开始前为每个学生保存完整快照（用于原题机伪等天赋的最终恢复）
    for(const s of activeStudents){
      try{
        s._talent_state = s._talent_state || {};
        if(!s._talent_state._mock_start_snapshot){
          s._talent_state._mock_start_snapshot = {
            thinking: Number(s.thinking || 0),
            coding: Number(s.coding || 0),
            mental: Number(s.mental || 0),
            constmental: s._talent_state.constmental || Number(s.mental || 0)
          };
        }
      }catch(e){ console.error('保存模拟赛快照失败', e); }
    }
