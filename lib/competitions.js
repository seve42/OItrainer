...
(function(global){
   ...
   */
  function generateSubtasks(totalScore, problemDifficulty){
    ...
    for(let i = 1; i <= numSubtasks; i++){
      ...
      subtasks.push({ score, difficulty });
    }
  ...
  class ContestSimulator {
    ...
    attemptSubtask(student, problem, subtask){
      // 若学生为激进，则只尝试最后一档（调用方会传入 subtask 为最后一档）
      ...
      // 成功率计算：基于难度差距的sigmoid函数
      const difficultyGap = effectiveAbility - subtask.difficulty;
      let baseProb = 1.0 / (1.0 + Math.exp(-difficultyGap / 15.0)); // sigmoid

      // 心理素质影响稳定性
      const stabilityFactor = 0.7 + 0.3 * (mental / 100.0);
      let finalProb = baseProb * stabilityFactor;

      // 记录原始最终概率（用于日志显示）
      const originalFinalProb = finalProb;

      // 新增机制：难度压制
      // 当题目难度大于选手综合能力的 2 倍时，将部分分通过概率降为原来的 35%
      try{
        if(problem && typeof problem.difficulty === 'number' && problem.difficulty > 2.0 * effectiveAbility){
          finalProb = originalFinalProb * 0.35;
          // 将触发信息写入比赛日志，包含原始概率与最终概率
          try{
            const origPct = (originalFinalProb * 100.0).toFixed(1);
            const finalPct = (finalProb * 100.0).toFixed(1);
            const msg = `${student.name} 在 T${(problem.id||0) + 1} 触发 难度压制：题目难度 ${problem.difficulty} > 2× 综合能力 ${effectiveAbility.toFixed(1)}，题目通过概率由 ${origPct}% 降至 ${finalPct}%`;
  ...
  function buildContestConfig(contestDef){
    ...
    for(let i = 0; i < contestDef.numProblems; i++){
      ...
      const problemDifficulty = baseDifficulty + (i * 20) - 10 + Math.floor(Math.random() * 20);

      // 生成部分分
      const subtasks = generateSubtasks(problemScore, problemDifficulty);
