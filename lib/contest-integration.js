...
(function(global){
  ...
  function handleMockContestResults(studentStates, config, isPurchased, diffIdx){
    ...
    for(let state of studentStates){
      ...
      if(actualKnowledgeGain > knowledgeCap * 1.01){ // 允许1%的误差
        const scale = knowledgeCap / actualKnowledgeGain;
        console.warn(`[模拟赛知识增幅超限] 学生:${s.name}, 上限:${knowledgeCap.toFixed(2)}, 实际:${actualKnowledgeGain.toFixed(2)}, 缩减比例:${scale.toFixed(3)}`);
