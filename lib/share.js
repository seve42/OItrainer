...
const ShareManager = (function() {
  ...
  function collectEndGameData() {
    try {
      ...
      // 获取开发者工具检测结果（优先从存档读取，确保数据一致性）
      let devToolsResult = null;
      try {
        console.log('[ShareManager] 检查 devToolsDetection 数据...');
        console.log('[ShareManager] gameData.devToolsDetection:', gameData.devToolsDetection);
        
        // 优先从存档中读取（确保使用已保存的数据）
        if (gameData.devToolsDetection) {
          console.log('[ShareManager] 从存档读取 devToolsDetection');
          devToolsResult = {
            detected: gameData.devToolsDetection.detected || false,
            tampered: false,
            firstDetectedTime: gameData.devToolsDetection.firstDetectedTime || 0,
            detectionCount: gameData.devToolsDetection.detectionCount || 0,
            trustLevel: gameData.devToolsDetection.detected ? 50 : 100,
            message: gameData.devToolsDetection.detected ? '⚠️ 检测到开发者工具' : '✓ 未检测到开发者工具'
          };
          
          // 验证完整性（检查是否有数据篡改）
          try {
            if (gameData.devToolsDetection.detected !== gameData.devToolsDetection.detectedBackup1 ||
                gameData.devToolsDetection.detected !== gameData.devToolsDetection.detectedBackup2) {
              devToolsResult.tampered = true;
              devToolsResult.message = '⚠️ 检测到数据异常';
              devToolsResult.trustLevel = 0;
              console.log('[ShareManager] 检测到数据篡改');
            }
          } catch (e) {
            console.error('[ShareManager] 验证完整性失败:', e);
          }
          
          console.log('[ShareManager] devToolsResult 构建完成:', devToolsResult);
        } else {
          console.warn('[ShareManager] 存档中没有 devToolsDetection 字段');
          
          // 备用：从检测器实时获取（用于游戏进行中）
          if (typeof window !== 'undefined' && window.DevToolsDetector && typeof window.DevToolsDetector.getResult === 'function') {
            devToolsResult = window.DevToolsDetector.getResult();
            console.log('[ShareManager] 从检测器实时获取 devTools 结果:', devToolsResult);
          } else {
            console.warn('[ShareManager] DevToolsDetector 不可用');
          }
        }
      } catch (e) {
        console.error('[ShareManager] 获取 DevTools 检测结果失败:', e);
      }
      
      console.log('[ShareManager] 最终 devToolsResult:', devToolsResult);

      // 构建分享数据对象（只包含必要的显示信息，减少数据量）
      const shareData = {
        version: '1.1', // 数据格式版本（增加了devTools字段）
        timestamp: Date.now(),
        gameState: {
          ...
          devTools: devToolsResult, // 添加开发者工具检测结果
