...
function parseResultCsvContent(raw){
  try{
    ...
    const threshold = (maxMatchId > 9) ? maxMatchId - 9 : 0;
    ...
    for(const row of rows){
      ...
      let recentProv = null; let recentMatch = -Infinity;
      for(const p of partsParsed){ if(p.matchId !== null && p.matchId > recentMatch && p.provinceId !== null){ recentMatch = p.matchId; recentProv = p.provinceId; } }
      if(recentProv === null || recentProv < 0 || recentProv >= provinceNames.length) continue;
      const provId = recentProv;
      _namePoolsByProvince[provId] = _namePoolsByProvince[provId] || [];
      _namePoolsByProvince[provId].push({ name, weight: Math.max(0, Number(score) || 0) });
      _globalNamePool.push({ name, weight: Math.max(0, Number(score) || 0) });
    }
...
function generateName(province=-1,used = new Set()){
  ...
  // console.log(provKeys);
  // console.log(1345);
  // console.log(_namePoolsByProvince);
  ...
  let oierdb_province;
  console.log(PROVINCES[province]['name']);
