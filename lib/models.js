...
class GameState {
  ...
  updateWeather(){
    ...
    try{
      ...
      // 游戏从第1周开始，对应9月（秋季开学），所以需要偏移8个月
      // week 1 -> 9月, week ~4 -> 12月, week ~8 -> 3月, week ~12 -> 6月, week ~16 -> 9月...
      let rawMonth = Math.floor(((this.week - 1) / Math.max(1, weeksPerYear)) * 12) + 1;
      let month = ((rawMonth + 8 - 1) % 12) + 1; // 偏移8个月，从9月开始
      ...
      // 游戏周数对应：第1周=9月初（秋季），第8周=12月（冬季），第16周=次年3月（春季），第24周=6月（夏季），第32周=9月（秋季）
      // 32周游戏对应一个学年：9月-次年6月，然后是暑假7-8月
      const weeksPerSeason = (SEASON_WEEKS || 32) / 4;
      const weekInYear = ((this.week - 1) % (SEASON_WEEKS || 32)) + 1;
      
      // 第1-8周：秋季（9-11月）
      if(weekInYear >= 1 && weekInYear <= weeksPerSeason){ 
        if(this.is_north) this.temperature = uniform(5,20); 
        else this.temperature = uniform(15,28); 
      }
      // 第9-16周：冬季（12-2月）
      else if(weekInYear > weeksPerSeason && weekInYear <= weeksPerSeason * 2){ 
        if(this.is_north) this.temperature = uniform(-10,5); 
        else this.temperature = uniform(5,15); 
      }
      // 第17-24周：春季（3-5月）
      else if(weekInYear > weeksPerSeason * 2 && weekInYear <= weeksPerSeason * 3){ 
        if(this.is_north) this.temperature = uniform(8,22); 
        else this.temperature = uniform(18,28); 
      }
      // 第25-32周：夏季（6-8月）
      else { 
        if(this.is_north) this.temperature = uniform(20,35); 
        else this.temperature = uniform(28,38); 
      }
