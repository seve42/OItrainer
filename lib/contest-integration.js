...
(function(global){
  ...
  function holdCompetitionModalNew(comp){
    ...
    if(eligibleStudents.length === 0){
      ...
      // 如果是第一轮赛季（halfIndex = 0），跳过比赛
      if(halfIndex === 0){
        if(typeof window.pushEvent === 'function'){
          window.pushEvent({
            name: comp.name + ' 跳过',
            description: '没有学生晋级，比赛自动跳过',
            week: game.week
          });
        }
        ...
        if(typeof window.pushEvent === 'function'){
          window.pushEvent({
            name: '晋级链断裂',
            description: `${comp.name}：没有学生晋级，游戏结束`,
            week: game.week
          });
        }
        ...
        try{
          const settlement = {
            ...
            events: game.events || [],
            ending: ending,
            noi_rankings: game.noi_rankings || []
          };
          localStorage.setItem('settlement', JSON.stringify(settlement));
        } catch(e){ console.error('Failed to save settlement:', e); }
        
        // 跳转到结算页面
        if(typeof window.showModal === 'function' && typeof window.closeModal === 'function'){
          window.showModal(`<h3>晋级链断裂</h3><div class="small">第二轮赛季${comp.name}无学生晋级，游戏结束。</div><div class="modal-actions" style="margin-top:8px"><button class="btn" onclick="(function(){ closeModal(); window.location.href='end.html'; })()">查看结算页面</button></div>`);
        } else {
  ...
  <td style="font-size:14px;font-weight:bold;">${(isFinite(Number(r.score)) ? Number(r.score).toFixed(1) : '0.0')}</td>
    ...
    // 如果页面提供 closeModal 函数，确保在关闭时清理任何临时标志
    try{
      if(typeof window.closeModal === 'function'){
        // wrap original closeModal to ensure default behavior remains
