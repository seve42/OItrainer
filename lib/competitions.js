...
(function(global){
  ...
  function generateSubtasks(totalScore, problemDifficulty, thinkingBase=null, codingBase=null, options={}){
    ...
    for(let i = 1; i <= numSubtasks; i++){
      ...
      if(numSubtasks === 1){
        ...
        // 难度递增：基础难度 + 递增量（仍然以 numSubtasks 分布）
  ...
  class ContestSimulator {
    ...
    simulateStudentTick(state){
      ...
      // 2. 尝试解题 - 智能选择子任务档位
      // 如果所有档位都尝试过，返回
      if(prob.currentSubtask >= prob.subtasks.length) {
        return;
      }
      ...
      if(success){
        ...
        // 标记该档位已尝试过（避免重复尝试同一档）
        if(!prob.attemptedSubtasks) prob.attemptedSubtasks = new Set();
        prob.attemptedSubtasks.add(subtaskIdxToTry);
        
        // 如果尝试的是最后一档或已AC，设置 currentSubtask 为末尾
        if(subtaskIdxToTry >= prob.subtasks.length - 1 || prob.solved){
          prob.currentSubtask = prob.subtasks.length;
        }
        ...
        // 未成功通过该档位，标记为已尝试（避免无限重试）
        if(!prob.attemptedSubtasks) prob.attemptedSubtasks = new Set();
        prob.attemptedSubtasks.add(subtaskIdxToTry);
        
        // 如果所有档位都尝试过，标记为完成
        if(prob.attemptedSubtasks.size >= prob.subtasks.length){
          prob.currentSubtask = prob.subtasks.length;
        }
    ...
    selectProblem(state, student){
      ...
      const scored = unsolved.map(p => {
        // 获取该题未尝试过的最简单档位作为难度参考
        const attemptedSubtasks = p.attemptedSubtasks || new Set();
        const availableSubtasks = p.subtasks.filter((st, idx) => !attemptedSubtasks.has(idx));
        const easiestSubtask = availableSubtasks.length > 0 
          ? availableSubtasks.reduce((min, st) => (!min || st.difficulty < min.difficulty) ? st : min, null)
          : p.subtasks[0]; // 兜底：使用第一档
        ...
        let baseScore = knowledge * 1.0 + ability - Math.abs(easiestSubtask.difficulty - ability) * 0.5;
    ...
    selectBestSubtask(student, prob){
      ...
      const attemptedSubtasks = prob.attemptedSubtasks || new Set();
      
      // 获取未尝试过的子任务
      const availableSubtasks = subtasks
        .map((st, idx) => ({ subtask: st, idx: idx }))
        .filter(item => !attemptedSubtasks.has(item.idx));
      
      if(availableSubtasks.length === 0){
        // 所有档位都尝试过，标记为完成
        prob.currentSubtask = subtasks.length;
        return subtasks.length - 1; // 返回最后一档作为兜底
      }
      ...
      const scored = availableSubtasks.map(item => {
        ...
        // 综合匹配度：能力略高于难度时得分最高
        // 如果能力远高于难度，得分会下降（浪费时间）
        // 如果能力低于难度，得分也会下降（通过率低）
        let matchScore = 0;
        
        // 思维匹配：能力高于难度5-20分时最优
        if(thinkingGap >= 5 && thinkingGap <= 20){
          matchScore += 100;
        } else if(thinkingGap > 20){
          matchScore += 100 - (thinkingGap - 20) * 2; // 能力过高，浪费时间
        } else if(thinkingGap >= -10){
          matchScore += 50 + thinkingGap * 5; // 稍低于难度，还有机会
        } else {
          matchScore += 10; // 能力远低于难度，得分很低
        }
        
        // 代码匹配：类似逻辑
        if(codingGap >= 5 && codingGap <= 20){
          matchScore += 100;
        } else if(codingGap > 20){
          matchScore += 100 - (codingGap - 20) * 2;
        } else if(codingGap >= -10){
          matchScore += 50 + codingGap * 5;
        } else {
          matchScore += 10;
        }
        
        // 分值权重：分值越高，吸引力越大（但不是唯一因素）
        const scoreWeight = st.score * 0.5;
        
        // 综合得分
        const totalScore = matchScore + scoreWeight;
    ...
    shouldSkipProblem(state, student){
      ...
      // 默认策略：思考时间过长且难度远超能力时跳题
      const prob = state.getProblem(state.currentTarget);
      if(!prob) return false;

      // 获取未尝试过的子任务中最简单的那个作为难度参考
      const attemptedSubtasks = prob.attemptedSubtasks || new Set();
      const availableSubtasks = prob.subtasks.filter((st, idx) => !attemptedSubtasks.has(idx));
      ...
      // 找到最简单的可用档位
      const easiestSubtask = availableSubtasks.reduce((min, st) => 
      ...
      if(difficultyGap > 40 && state.thinkingTime >= 30){
        return getRandom() < 0.4; // 40%概率跳题
      }

      // 思考时间超过60分钟
      if(state.thinkingTime >= 60){
        return getRandom() < 0.6; // 60%概率跳题
      }
