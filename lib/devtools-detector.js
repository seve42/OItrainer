...
(function() {
  ...
  function setupProtection() {
    if (typeof window !== 'undefined' && window.game) {
      try {
        ...
        originalConsole.error.call(console, '[Console Detector] 保护设置失败:', e);
      }
    }
  }
  ...
  function detectByConsoleHistory() {
    try {
      ...
      function checkCallStack() {
        try {
          ...
          for (let i = 0; i < Math.min(5, stackLines.length); i++) {
            ...
            if (line.includes('devtools-detector') || line.includes('console-detector')) {
              return false;
            }
          }
      ...
      // 包装console方法 - 使用原始方法避免递归
      console.log = function() {
        if (checkCallStack()) {
          originalConsole.log.call(console, '[Console Detector] 检测到控制台代码执行 (log)');
          updateDetectionState(true);
        }
      ...
      console.warn = function() {
        if (checkCallStack()) {
          originalConsole.log.call(console, '[Console Detector] 检测到控制台代码执行 (warn)');
          updateDetectionState(true);
        }
      ...
      console.info = function() {
        if (checkCallStack()) {
          originalConsole.log.call(console, '[Console Detector] 检测到控制台代码执行 (info)');
          updateDetectionState(true);
        }
        return originalConsole.info.apply(console, arguments);
      };
      
      // 不包装console.error，避免无限递归
      
    } catch (e) {
      originalConsole.error.call(console, '[Console Detector] Console历史检测设置失败:', e);
    }
  }
  ...
  function detectByGlobalObjectAccess() {
    try {
            ...
            Object.defineProperty(originalGame, prop, {
              get: function() {
                ...
                try {
                  ...
                  // 排除检测器自身的调用
                  let hasDetectorCall = false;
                  for (let i = 0; i < stackLines.length; i++) {
                    const line = stackLines[i].toLowerCase();
                    if (line.includes('devtools-detector') || line.includes('console-detector')) {
                      hasDetectorCall = true;
                      break;
                    }
                  }
                  ...
                  if (!hasDetectorCall) {
                    for (let i = 0; i < Math.min(5, stackLines.length); i++) {
                      ...
                      if ((line.includes('<anonymous>') || 
                           line.includes('eval') ||
                           (line.includes('at') && !line.includes('.js') && !line.includes('.html')))) {
                        originalConsole.log.call(console, `[Console Detector] 检测到从控制台访问 game.${prop}`);
                        updateDetectionState(true);
                        break;
                      }
                    }
                  ...
                  if (!hasDetectorCall) {
                    for (let i = 0; i < Math.min(5, stackLines.length); i++) {
                      ...
                      if ((line.includes('<anonymous>') || 
                           line.includes('eval') ||
                           (line.includes('at') && !line.includes('.js') && !line.includes('.html')))) {
                        originalConsole.log.call(console, `[Console Detector] 检测到从控制台修改 game.${prop}`);
                        updateDetectionState(true);
                        break;
                      }
                    }
      ...
      originalConsole.error.call(console, '[Console Detector] 全局对象访问监控设置失败:', e);
    }
  }
  ...
  function detectByEvalUsage() {
    try {
      ...
      window.eval = function(code) {
        originalConsole.log.call(console, '[Console Detector] 检测到eval调用');
        updateDetectionState(true);
      ...
      window.Function = function() {
        originalConsole.log.call(console, '[Console Detector] 检测到Function构造器调用');
        updateDetectionState(true);
      ...
      originalConsole.error.call(console, '[Console Detector] Eval检测设置失败:', e);
    }
  }
  ...
  function detectByStorageModification() {
    try {
            ...
            if (!hasDetectorCall) {
              for (let i = 0; i < Math.min(5, stackLines.length); i++) {
                ...
                if ((line.includes('<anonymous>') || 
                     line.includes('eval') ||
                     (line.includes('at') && !line.includes('.js') && !line.includes('.html')))) {
                  originalConsole.log.call(console, `[Console Detector] 检测到从控制台修改localStorage: ${key}`);
                  updateDetectionState(true);
                  break;
                }
              }
      ...
      originalConsole.error.call(console, '[Console Detector] Storage监控设置失败:', e);
    }
  }
  ...
  function startDetector() {
    originalConsole.log.call(console, '[Console Detector] 检测器已启动');
    
    // 检测浏览器类型 - 不使用已弃用的InstallTrigger
    const isFirefox = navigator.userAgent.toLowerCase().includes('firefox');
    const isChrome = !!window.chrome && (!!window.chrome.webstore || !!window.chrome.runtime);
    
    originalConsole.log.call(console, '[Console Detector] 浏览器信息:', {
  ...
  function stopDetector() {
    originalConsole.log.call(console, '[Console Detector] 检测器已停止');
