    ...
    registerDefaultTalents(game, utils){
        ...
        handler: function(student, eventName, ctx){
          try{
            ...
            if(eventName === 'contest_start' || eventName === 'mock_start'){
              if(!student._talent_backup['激进_original']){
                // 备份原始值（比赛开始前的真实值）
                student._talent_backup['激进_original'] = { 
                  thinking: Number(student.thinking || 0), 
                  constmental: (student._talent_state && student._talent_state.constmental) || Number(student.mental || 0) 
                };
                // 在原始值基础上应用修改
                ...
                // 如果是模拟赛且有原题机伪，更新原题机伪的快照以包含激进修改后的值作为"模拟赛开始前"的基准
                // 这样原题机伪在结束时可以正确恢复到激进修改前的值
                if(eventName === 'mock_start' && student.hasTalent && student.hasTalent('原题机（伪）')){
                  // 不做任何事，让原题机伪在 mock_start 时记录完整快照（在激进之前）
                  // 由于天赋按字母顺序执行，"原题机（伪）"会在"激进"之前执行
                }
          ...
          handler: function(student, eventName, ctx){
            try{
              if(eventName === 'mock_start'){
                student._talent_state = student._talent_state || {};
                // 快照已在 contest-integration.js 中提前保存，这里只需应用能力倍增
                // 在当前值基础上翻倍（叠加效果）
              ...
              if(eventName === 'mock_end' || eventName === 'mock_contest_finish'){
                // 恢复到模拟赛开始前的完全快照（已在 contest-integration.js 中保存）
                if(student._talent_state && student._talent_state._mock_start_snapshot){
                  const snap = student._talent_state._mock_start_snapshot;
                  student.thinking = snap.thinking; 
                  student.coding = snap.coding; 
                  student.mental = snap.mental;
                  if(student._talent_state) {
                    student._talent_state.constmental = snap.constmental;
                  }
                  // 不要删除快照，让 __talent_cleanup__ 统一清理
                  student._talent_state['原题机（伪）'] = false;
                  
                  // 清理所有模拟赛相关的天赋备份（包括激进等）
                  if(student._talent_backup){
                    // 清理激进的备份，因为我们已经恢复到激进修改之前的状态
                    delete student._talent_backup['激进_original'];
                    delete student._talent_backup['原题机（伪）_original'];
                  }
      ...
      this.registerTalent({
        ...
        handler: function(student, eventName, ctx){
          ...
          try{
            // 对于模拟赛结束，清理模拟赛快照（原题机伪已经使用过了）
            if((eventName === 'mock_end' || eventName === 'mock_contest_finish') && student._talent_state){
              delete student._talent_state._mock_start_snapshot;
            }
            ...
            if(student._talent_backup){
              for(const k of Object.keys(student._talent_backup)){
                // 跳过已经被原题机伪清理过的备份
                if(k === '原题机（伪）_original' || k === '激进_original') continue;
